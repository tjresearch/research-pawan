...
class Base_Agent(object):
    ...
    def __init__(self, config):
        ...
        if type(self.environment) == list:
            ...
            self.state_size = 128
            self.max_episode_steps = [10000, 10000, 10000]
    ...
    def get_lowest_possible_episode_score(self):
        ...
        if self.environment_title == "MountainCarContinuous": return -200
        return None
    ...
    def get_score_required_to_win(self):
        ...
        if self.environment_title == "MountainCarContinuous": return 90
    ...
    def get_trials(self):
        ...
        if self.environment_title == "MountainCarContinuous": return 10
        return 100

    def get_episode_timestep_limit(self):
        if self.environment_title == "MountainCarContinuous": return 200
        return 9999
    ...
    def reset_game_j(self, n):
        """Resets the game information so we are ready to play a new episode"""
        self.environment[n].seed(self.config.seed)
        self.state = self.environment[n].reset()
        self.next_state = None
        self.action = None
        self.reward = None
        self.done = False
    ...
    def run_n_episodes(self, num_episodes=None, show_whether_achieved_goal=True, save_and_print_results=True):
        ...
        if isinstance(self.environment,list):
            ...
            if self.config.save_results: self.save_checkpoint()
    ...
    def conduct_action_j(self, action, n):
        """Conducts an action in the environment"""
        self.next_state, self.reward, self.done, _ = self.environment[n].step(action)
        self.total_episode_score_so_far += self.reward
        if self.hyperparameters["clip_rewards"]: self.reward =  max(min(self.reward, 1.0), -1.0)
    ...
    def save_result_j(self, n):
        """Saves the result of an episode of the game"""
        self.game_full_episode_scores[n].append(self.total_episode_score_so_far)
        self.rolling_results[n].append(np.mean(self.game_full_episode_scores[n][-1 * self.rolling_score_window:]))
        self.save_max_result_seen_j(n)

    def save_max_result_seen_j(self, n):
        """Updates the best episode result seen so far"""
        if self.game_full_episode_scores[n][-1] > self.max_episode_score_seen[n]:
            self.max_episode_score_seen[n] = self.game_full_episode_scores[n][-1]

        if self.rolling_results[n][-1] > self.max_rolling_score_seen[n]:
            if len(self.rolling_results) > self.rolling_score_window:
                self.max_rolling_score_seen[n] = self.rolling_results[n][-1]
    ...
    def show_whether_achieved_goal_j(self, n):
        """Prints out whether the agent achieved the environment target goal"""
        index_achieved_goal = self.achieved_required_score_at_index_j(n)
        print(" ")
        if index_achieved_goal == -1: #this means agent never achieved goal
            print("\033[91m" + "\033[1m" +
                  "{} did not achieve required score \n".format(self.agent_name) +
    ...
    def achieved_required_score_at_index_j(self, n):
        """Returns the episode at which agent achieved goal or -1 if it never achieved it"""
        for ix, score in enumerate(self.rolling_results[n]):
            if score >= self.average_score_required_to_win[n]:
                return ix
        return -1
